...
import type { Alert, AlertSeverity, MonitorConfig, DashboardMetrics } from './types';
...
function buildDailyReportMessage(metrics: DashboardMetrics): string {
...
async function persistAlert(alert: Alert): Promise<void> {
  ...
  try {
    await db.collection('monitor_alerts').doc(alert.id).set({
      ...alert,
      resolvedAt: null,
...
export async function dispatchAlerts(
  ...
  if (!config.whatsapp.enabled || alerts.length === 0) return;
  ...
  for (const alert of alerts) {
    ...
    const recipients = config.whatsapp.recipients.filter(r => {
      ...
      if (alert.severity === 'warning') return r.role === 'superadmin' || r.role === 'admin';
