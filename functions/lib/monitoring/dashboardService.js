...
exports.saveCurrentMetrics = saveCurrentMetrics;
exports.saveHistoricalPoint = saveHistoricalPoint;
exports.getCurrentMetrics = getCurrentMetrics;
exports.getHistory = getHistory;
const firestore_1 = require("firebase-admin/firestore");
const db = () => (0, firestore_1.getFirestore)();
// ── Calcula health score de 0 a 100 ─────────────────────────
function calculateHealthScore(technical, operational, memorial) {
    let score = 100;
    // Tecnico (peso 50)
    if (technical.appStatus === 'offline')
        score -= 50;
    else if (technical.appStatus === 'degraded')
        score -= 20;
    if (technical.firestoreLatencyMs === -1)
        score -= 15;
    else if (technical.firestoreLatencyMs > 2000)
        score -= 5;
    if (technical.failedLogins24h > 20)
        score -= 10;
    if (technical.functionsErrors24h > 10)
        score -= 5;
    // Operacional (peso 30)
    if (operational.servicosAtrasados > 10)
        score -= 15;
    else if (operational.servicosAtrasados > 5)
        score -= 8;
    if (operational.comunicadosObitoSemValidar > 5)
        score -= 10;
    else if (operational.comunicadosObitoSemValidar > 0)
        score -= 5;
    if (operational.gestoresAtivos === 0)
        score -= 10;
    // Memorial (peso 20)
    if (memorial.jazigosSemManutencao90d > 20)
        score -= 10;
    else if (memorial.jazigosSemManutencao90d > 5)
        score -= 5;
    if (memorial.planosFunerariosVencendo30d > 10)
        score -= 5;
    return Math.max(0, Math.min(100, score));
}
// ── Consolida todos os alertas abertos ──────────────────────
function consolidateAlerts(technical, operational, memorial) {
    return [
        ...technical.alerts,
        ...operational.alerts,
        ...memorial.alerts,
...
async function saveCurrentMetrics(technical, operational, memorial) {
    ...
    try {
        ...
        console.log(`[DashboardService] Metricas salvas. Health Score: ${systemHealthScore}/100 | Alertas: ${alertsOpen.length}`);
    }
    catch (err) {
...
async function saveHistoricalPoint(metrics) {
